...
## Integration with `ghostd` Runtime

The value of reflection depends on how its outputs are used during active execution. This section outlines how the daemon (`ghostd`) integrates reflection data into its runtime loop.

### Heuristic Loading

- Reflections output structured heuristic entries (YAML or markdown frontmatter)
- On daemon startup or model handoff, the `resolver` ritual loads active heuristics
- Relevant patterns are injected into seed prompts or runtime logic

### Confidence-Weighted Influence

- High-confidence heuristics may directly alter:
  - Ritual preconditions
  - Queue ordering logic
  - Prompt formatting (e.g. verbosity levels)
- Medium-confidence heuristics may surface as suggestions or soft nudges
- Low-confidence or deprecated heuristics are excluded from runtime

### Deprecation and Filtering

- A `ghost_reflection/archive.md` index tracks stale or rejected heuristics
- The daemon avoids loading any heuristic marked `deprecated: true` or with decayed confidence
- Optionally, reflection data older than a time threshold is archived automatically

### Runtime Use Cases

- Selecting between multiple ritual paths based on past failure/success
- Adapting response structure when user feedback trends change
- Suppressing known failure patterns or dead ends before execution
